...
const LineChart = ({
  ...
  const calculateHeight = () => {
    let yMax = data.reduce((acc, cur) => {
      return cur[y_key] > acc ? cur[y_key] : acc;
    }, 0);
    let yMin = data.reduce((acc, cur) => {
      return cur[y_key] < acc ? cur[y_key] : acc;
    }, yMax);
   let checkYMax =-1;
    ...
    let min = 0;
    let actual_chart_height = containerHeight - y_margin * 2;
    let gap_between_ticks = actual_chart_height / (data.length - 1);
    let y_value_gap = Math.round((yMax - min) / (data.length - 1));
  
    return { yMax, yMin, gap_between_ticks, y_value_gap, min,checkYMax };
  };
  ...
  const render_line_circles = () => {
    ...
    const {gap_between_ticks: y_gap, yMax, y_value_gap,checkYMax} = calculateHeight();
    // console.log("inside render circles , yMax ",yMax)
    if (checkYMax === 0) {
      return null;
    }
    const {
  ...
  const render_line = () => {
    const {checkYMax} = calculateHeight();
    if(checkYMax===0) return null;
